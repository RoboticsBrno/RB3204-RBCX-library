...
ArmBuilder& ArmBuilder::body(
    Arm::CoordType height_mm, Arm::CoordType radius_mm) {
...
BoneBuilder ArmBuilder::bone(uint8_t servo_id, Arm::CoordType length_mm) {
    std::shared_ptr<Arm::BoneDefinition> bone(
        new Arm::BoneDefinition(servo_id, length_mm));
...
BoneBuilder& BoneBuilder::calcServoAng(
    std::function<Angle(Angle abs, Angle rel)> func) {
    m_def->calcServoAng = func;
    return *this;
}

BoneBuilder& BoneBuilder::calcAbsAng(
    std::function<Angle(Angle servoAng)> func) {
...
bool Arm::syncBonesWithServos() {
    ...
    for (size_t i = 0; i < m_bones.size(); ++i) {
        ...
        if (prev == nullptr) {
            ...
            bone->relAngle
                = Arm::clamp(bone->def.calcAbsAng(pos) - prev->absAngle);
        }
        bone->updatePos(prev);
        prev = bone;
    }
...
bool Arm::solveIteration(
    Arm::CoordType target_x, Arm::CoordType target_y, bool& modified) {
    updateBones();
    ...
    if (target_x < m_def.body_radius - m_def.arm_offset_x) {
        ...
        target_y = std::min(
            target_y, CoordType(m_def.arm_offset_y + m_def.body_height));
    }
    ...
    for (int32_t ii = int32_t(m_bones.size()) - 1; ii >= 0; --ii) {
        ...
        AngleType to_target_mag
            = sqrt(to_target_x * to_target_x + to_target_y * to_target_y);
        ...
        if (end_target_mag <= AngleType(0.0001)) {
            ...
            cos_rot_ang = (to_end_x * to_target_x + to_end_y * to_target_y)
                / end_target_mag;
            sin_rot_ang = (to_end_x * to_target_y - to_end_y * to_target_x)
                / end_target_mag;
        }

        // Clamp the cosine into range when computing the angle (might be out of range
        // due to floating point error).
        AngleType rot_ang = acos(
            std::max(AngleType(-1), std::min(AngleType(1), cos_rot_ang)));
        if (sin_rot_ang < 0)
            rot_ang = -rot_ang;
        ...
        end_x
            = roundCoord(bx + cos_rot_ang * to_end_x - sin_rot_ang * to_end_y);
        end_y
            = roundCoord(by + sin_rot_ang * to_end_x + cos_rot_ang * to_end_y);
    ...
    for (size_t i = 0; i < m_bones.size(); ++i) {
        ...
        if (i > 0) {
            ...
            if (diff < b.def.base_rel_min.rad()) {
                base.absAngle = Angle::rad(Angle::_T(
                    clamp(angle - AngleType(b.def.base_rel_min.rad()))));
            } else if (diff > b.def.base_rel_max.rad()) {
                base.absAngle = Angle::rad(Angle::_T(
                    clamp(angle - AngleType(b.def.base_rel_max.rad()))));
            }
        }
...
void Arm::fixBodyCollision() {
    ...
    while (isInBody(end.x, end.y)) {
        ...
        if (newang.rad() > base.def.rel_max.rad()
            || newang.rad() < base.def.rel_min.rad())
            return;
