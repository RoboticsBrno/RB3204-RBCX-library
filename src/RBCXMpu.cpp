#include "RBCXManager.h"
#include "RBCXMpu.h"
#include "Arduino.h"

#include "rbcx.pb.h"

namespace rb {

Mpu::Mpu() {
    preInterval = millis();
}

Mpu::~Mpu() {}


void Mpu::setState(const CoprocStat_MpuStat& msg) {
    // printf("DEBUG MPU acc: x:%d; y:%d; z:%d | gyro: x:%d; y:%d; z:%d\n", msg.accel.x, msg.accel.y, msg.accel.z, msg.gyro.x, msg.gyro.y, msg.gyro.z);
    if(firstRead) {
        firstRead = false;
        calculateAcc(msg.accel);
        calculateGyro(msg.gyro);
        calcOffsets();
    }

    calculateAcc(msg.accel);
    calculateGyro(msg.gyro);
    calculateAngle();

    // printf("DEBUG MPU acc: x:%2.3f; y:%2.3f; z:%2.3f | gyro: x:%2.3f; y:%2.3f; z:%2.3f\n", m_mpuMotion.accel.x, m_mpuMotion.accel.y, m_mpuMotion.accel.z, m_mpuMotion.gyro.x, m_mpuMotion.gyro.y, m_mpuMotion.gyro.z);
}

void Mpu::calcOffsets() {
    // MpuMotion6 offset;

    m_mpuMotionOffset.accel.x = m_mpuMotion.accel.x / CALIB_OFFSET_NB_MES;
    m_mpuMotionOffset.accel.y = m_mpuMotion.accel.y / CALIB_OFFSET_NB_MES;
    m_mpuMotionOffset.accel.z = (m_mpuMotion.accel.z - 1.0) / CALIB_OFFSET_NB_MES;

    m_mpuMotionOffset.gyro.x = m_mpuMotion.gyro.x / CALIB_OFFSET_NB_MES;
    m_mpuMotionOffset.gyro.y = m_mpuMotion.gyro.y / CALIB_OFFSET_NB_MES;
    m_mpuMotionOffset.gyro.z = m_mpuMotion.gyro.z / CALIB_OFFSET_NB_MES;

}

void Mpu::calculateAcc(const CoprocStat_MpuVector& accel) {
    m_mpuMotion.accel.x = ((float)accel.x * MPU6050_ACCEL_FACTOR_2) - m_mpuMotionOffset.accel.x;
    m_mpuMotion.accel.y = ((float)accel.y * MPU6050_ACCEL_FACTOR_2) - m_mpuMotionOffset.accel.y;
    m_mpuMotion.accel.z = ((float)accel.z * MPU6050_ACCEL_FACTOR_2) - m_mpuMotionOffset.accel.z;
    // m_mpuMotion.accel.x = (float)accel.x;
    // m_mpuMotion.accel.y = (float)accel.y;
    // m_mpuMotion.accel.z = (float)accel.z;
}


void Mpu::calculateGyro(const CoprocStat_MpuVector& gyro) {
    m_mpuMotion.gyro.x = ((float)gyro.x * MPU_GYRO_FACTOR_250) - m_mpuMotionOffset.gyro.x;
    m_mpuMotion.gyro.y = ((float)gyro.y * MPU_GYRO_FACTOR_250) - m_mpuMotionOffset.gyro.y;
    m_mpuMotion.gyro.z = ((float)gyro.z * MPU_GYRO_FACTOR_250) - m_mpuMotionOffset.gyro.z;
    // m_mpuMotion.gyro.x = (float)gyro.x;
    // m_mpuMotion.gyro.y = (float)gyro.y;
    // m_mpuMotion.gyro.z = (float)gyro.z;
}

void Mpu::calculateAngle() {
    m_mpuMotion.angle.x = 0;
    m_mpuMotion.angle.y = 0;
    m_mpuMotion.angle.z = 0;

    // estimate tilt angles: this is an approximation for small angles!
    float sgZ = (m_mpuMotion.accel.z>=0)-(m_mpuMotion.accel.z<0); // allow one angle to go from -180° to +180°
    // angleAccX =   atan2(accY, sgZ*sqrt(accZ*accZ + accX*accX)) * RAD_2_DEG; // [-180°,+180°]
    m_mpuMotion.angleAcc.x =   atan2(m_mpuMotion.accel.y, sgZ*sqrt(m_mpuMotion.accel.z*m_mpuMotion.accel.z + m_mpuMotion.accel.x*m_mpuMotion.accel.x)) * RAD_2_DEG; // [-180°,+180°]
    
    // angleAccY = - atan2(accX,     sqrt(accZ*accZ + accY*accY)) * RAD_2_DEG; // [- 90°,+ 90°]
    m_mpuMotion.angleAcc.y = - atan2(m_mpuMotion.accel.x,     sqrt(m_mpuMotion.accel.z*m_mpuMotion.accel.z + m_mpuMotion.accel.y*m_mpuMotion.accel.y)) * RAD_2_DEG; // [- 90°,+ 90°]

    // unsigned long Tnew = millis();
    unsigned long Tnew = millis();
    float dt = (Tnew - preInterval) * 1e-3;
    preInterval = Tnew;

    // Correctly wrap X and Y angles (special thanks to Edgar Bonet!)
    // https://github.com/gabriel-milan/TinyMPU6050/issues/6
    // angleX = wrap(filterGyroCoef*(angleAccX + wrap(angleX +     gyroX*dt - angleAccX,180)) + (1.0-filterGyroCoef)*angleAccX,180);
    m_mpuMotion.angle.x = wrap((m_mpuMotion.angleAcc.x + wrap(m_mpuMotion.angle.x +     m_mpuMotion.gyro.x*dt - m_mpuMotion.angleAcc.x,180)) + m_mpuMotion.angleAcc.x,180);
    
    // angleY = wrap(filterGyroCoef*(angleAccY + wrap(angleY + sgZ*gyroY*dt - angleAccY, 90)) + (1.0-filterGyroCoef)*angleAccY, 90);
    m_mpuMotion.angle.y = wrap((m_mpuMotion.angleAcc.y + wrap(m_mpuMotion.angle.y + sgZ*m_mpuMotion.gyro.y*dt - m_mpuMotion.angleAcc.y, 90)) + m_mpuMotion.angleAcc.y, 90);
    
    // angleZ += gyroZ*dt; // not wrapped (to do???)
    m_mpuMotion.angle.z += m_mpuMotion.gyro.z*dt; // not wrapped (to do???)
}

/* Wrap an angle in the range [-limit,+limit] (special thanks to Edgar Bonet!) */
float Mpu::wrap(float angle,float limit) {
  while (angle >  limit) angle -= 2*limit;
  while (angle < -limit) angle += 2*limit;
  return angle;
}

MpuVector Mpu::getAcc() {
    return m_mpuMotion.accel;
}
float Mpu::getAccX() {
    return m_mpuMotion.accel.x;
}
float Mpu::getAccY() {
    return m_mpuMotion.accel.y;
}
float Mpu::getAccZ() {
    return m_mpuMotion.accel.z;
}


MpuVector Mpu::getGyro() {
    return m_mpuMotion.gyro;
}
float Mpu::getGyroX() {
    return m_mpuMotion.gyro.x;
}
float Mpu::getGyroY() {
    return m_mpuMotion.gyro.y;
}
float Mpu::getGyroZ() {
    return m_mpuMotion.gyro.z;
}


MpuVector Mpu::getAngle() {
    return m_mpuMotion.angle;
}
float Mpu::getAngleX() {
    return m_mpuMotion.angle.x;
}
float Mpu::getAngleY() {
    return m_mpuMotion.angle.y;
}
float Mpu::getAngleZ() {
    return m_mpuMotion.angle.z;
}


}; //namespace rb
